...
namespace StrattonOakmontServices.Sql
{
    public class SqlSecurityRepository : ISecurityRepository
    {
        ...
        public IEnumerable<UserStock> GetUserStock(string Name)
        {
            return _context.UserStocks.Include(x => x.User).Include(x => x.Stoсk).ThenInclude(x => x.CompanySec).Where(x => x.User.UserName == Name);
        }
